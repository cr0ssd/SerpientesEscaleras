...
//normalsitesientesolitaymextranas
    ...
    void play(Tile** board, int boardSize) override {
        ...
        while (turnCount < maxTurns) {
            ...
            if (targetPosition <= boardSize) {
                targetPosition = board[targetPosition - 1]->applyEffect(targetPosition);
            }

            cout << turnCount << " " << currentPlayer + 1 << " " << playerPositions[currentPlayer] << " "
                 << diceRoll << " " << board[targetPosition - 1]->getType() << " " << targetPosition << endl;
